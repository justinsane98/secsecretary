...
const rssUrl = "https://www.sec.gov/rss/news/press.xml";
const sourceUrl = "https://www.sec.gov/news/pressreleases";

const [todaysNews, setTodaysNews] = useState(0)
  ...
  parser.parseURL(rssUrl, function(err, feed) {
    ...
    feed.items.forEach(function(entry) {
      ...
      if((entryDay == searchDay) && entryMonth === searchMonth){
        subset.push (
          <li className="768px:flex-col 768px:w-1/2 768px:px-8 py-4" key={entry.title}>
            <a className="text-large text-navy font-bold hover:underline" href={entry.link}>{entry.title}</a>
            <p className="opacity-90">{entry.content}</p>
            <p className="text-sm opacity-25">{entryTimeString}</p>
          </li>
        );
      }
    })
    setTodaysNews(subset)
  })
}, [])

return (
  <>
      <section className={"mb-12 " + (todaysNews.length > 0 ? "block" : "hidden" )}>
          ...
          <h2 className="text-2xl border-b border-black-25 pb-2">Press Releases</h2>
